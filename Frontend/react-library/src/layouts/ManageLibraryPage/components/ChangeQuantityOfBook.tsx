...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props) => {
    ...
    const increaseQuantity = async () => {
        const url = `http://13.51.160.178:8080/api/admin/secure/increase/book/quantity?bookId=${props.book?.id}`;
    ...
    const decreaseQuantity = async () => {
        const url = `http://13.51.160.178:8080/api/admin/secure/decrease/book/quantity?bookId=${props.book?.id}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    const deleteBook = async () => {
        const url = `http://13.51.160.178:8080/api/admin/secure/delete/book?bookId=${props.book?.id}`;
        const requestOptions = {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
